function [  ] = write_output(rec, IMs, outputDir, outputFile, getTimeSeries, Filename, dirLocation, compNum)
...
for i = 1 : length(rec)
    if size(Filename,2) == 1 % only one ground motion component
        fprintf(fin,'%d \t %d \t %6.2f \t %d \t %s \t %s \n',i,rec(i),IMs.scaleFac(i), compNum(rec(i)), char(Filename{rec(i)}),[char(dirLocation{rec(i)}) char(Filename{rec(i)})]); % Print relevant outputs
    else 
